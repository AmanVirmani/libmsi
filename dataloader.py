...
class DataLoader:
    ...
    """
    def __init__(self, filenames=[], binSize=1, split_ratio=0.75):
        """
    ...
    def load_files(self):
        ...
        for file in self.filenames:
            files.append(libmsi.Imzml(file, self.binSize))
        return files
    ...
    def perform_nmf(self, n_components=15, type='all', input_data = None, n_iter=50):
        ...
        if type == 'all':
            self.nmf = newNMF()
            data = np.vstack([file.imzml_2d_array for file in self.train_data]) if input_data is None else input_data
            X_transformed, components, info = self.nmf.run(data, n_components,
            max_iter = n_iter)
            start_idx = 0
            for i in range(len(self.train_data)):
                data_list.append(X_transformed[start_idx:start_idx + len(self.train_data[i].imzml_2d_array)])
                start_idx += len(self.train_data[i].imzml_2d_array)
        elif type == 'each':
            for file in self.files:
    ...
    def plot_cluster_reprojection(self, cluster_list, type='each', fn_prefix='cph_'):
        ...
        for i, cluster_dict in enumerate(cluster_list):
            ...
            if type == 'each':
                ...
                cluster_mean_spectra = cluster_dict['clusters'].dot(self.nmf.components_)
