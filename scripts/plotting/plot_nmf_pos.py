...
if __name__ == "__main__":
    ...
    fig, ax = plt.subplots(len(fnames), 20)
    for i, data in enumerate(nmf_data["nmf_data_list"]):
        ...
        for j in range(data.shape[-1]):
            ax[i, j].imshow(msi_data[:, :, j], cmap=customCmap.Black2Green).set_interpolation(
                "none"
            )
            ax[i, j].set_title('dataset {0}, nmf component {1}'.format(i, 1 + j), fontsize=2)
    plt.show()
    # plt.savefig("NMF_data_for_pos_mode.png")
    pass