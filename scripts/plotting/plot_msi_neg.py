...
if __name__ == "__main__":
    ...
    fn = "/home/avirmani/projects/aim-hi/data/DMSI0004_F893CPH_LipidNeg_IMZML/dmsi0004_0_05.pickle"
    binSize = 0.05
    fileNames = [os.path.join(data_path, path) for path in all_paths]
    # dataset = DataLoader(fileNames, binSize)
    dataset = DataLoader([fn], binSize)
    ...
    def plot_dataset(mz):
        for i, ax_i in enumerate(ax.reshape(-1)):
            ...
            print(idx)
            im = ax_i.imshow(
                dataset.files[i].imzml_array[:, :, idx],
