...
import scripts.segmentation as sgm
...
if __name__ == "__main__":
    ...
    split = 1
    pca_comp = 20
    fnames = [os.path.join(data_dir, path) for path in imzml_paths]
    dataset = DataLoader(fnames, binSize, split, store=500)
    ...
    nmf_3d_data = []
    curr = 0
    for i, path in enumerate(imzml_paths):
        pca_2d_data = pca_data[curr : curr + len(dataset.files[i].coordinates)]
        pca_3d_data.append(dataset.files[i].reconstruct_3d_array(pca_2d_data))
        curr = curr + len(dataset.files[i].coordinates)
        nmf_2d_data = nmf_data["nmf_data_list"][i]
        nmf_3d_data.append(dataset.files[i].reconstruct_3d_array(nmf_2d_data))
    ...
    he_gold = (
        "/home/avirmani/projects/aim-hi/data/HE AIM-HI Colons Negative/DMSI0002HE.tif"
    )
    he_img = cv2.imread(he_gold)
    he_img = cv2.resize(he_img, (x, y))
    segments = sgm.make_image_segments(he_img)
    ...
    axes[2, 0].set(xlabel="PCA component Image")
    ...
    for i, idx in enumerate(best_nmf_indices):
        ...
        axes[i, 1].imshow(
            nmf_3d_data[4][:, :, idx], cmap=cmap_array_spatial[i]
        ...
        # axes[i, 0].plot(
        #     np.arange(starting_mz, starting_mz + nmf_components.shape[-1]),
        #     nmf_components[idx],
        #     cmap_array_spectra[i],
        #     linewidth=1,
        # )
        axes[i, 0].imshow(
            pca_3d_data[4][:, :, idx], cmap=cmap_array_spatial[i]
        ).set_interpolation("none")
