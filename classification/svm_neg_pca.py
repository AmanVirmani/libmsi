...
def generate_patches(
    ...
    for file in data_list:
        nmf_data_3d = file.reconstruct_3d_array(
            nmf_data[curr : curr + len(file.imzml_2d_array)]
        )
        ...
        if random_patches:
            ...
            while count_patch < n_patches:
                ...
                patch = nmf_data_3d[
                    row : row + patch_size[0], col : col + patch_size[1]
                ]
                if (
                    patch.shape[:2] != patch_size
                    or not np.count_nonzero(patch, axis=2).all()
                ):
                    continue
...
def generate_patch_data(
    ...
    train_patches = generate_patches(
        nmf_data,
        imzml_data.train_data,
        patch_size=patch_size,
        random_patches=True,
        random_state=42,
        n_patches=1000,
    )
    test_patches = generate_patches(
        nmf_data,
        imzml_data.val_data,
        patch_size=patch_size,
        random_patches=True,
...
def fit_best_svm(patches, labels, test_size=None, param_grid=None):
    ...
    grid = GridSearchCV(
        SVC(), param_grid, n_jobs=-1, refit=True, scoring="accuracy", verbose=2
    )
...
if __name__ == "__main__":
    ...
    msi_naive = DataLoader(
        [os.path.join(data_path, naive) for naive in naive_paths], 0.75
    )

    cph_data = np.vstack([file.imzml_2d_array for file in msi_cph.train_data])[:, :500]
    naive_data = np.vstack([file.imzml_2d_array for file in msi_naive.train_data])[
        :, :500
    ]
    ...
    # n_comp_list = np.hstack([np.arange(1, 5), np.arange(5, 31, 5)])
    # n_comp_list = [7, 9, 11, 13, 17, 19, 21, 23, 27, 29]
    n_comp_list = [19]
    acc_list = []
    for n_comp in n_comp_list:
        pca = PCA(n_components=n_comp)
        pca.fit(data)
        cph_nmf_data = pca.transform(cph_data)
        naive_nmf_data = pca.transform(naive_data)

        ## make roi of dimension (15, 15)
        for patch_dim in np.arange(5, 31, 5):
            ...
            (
                ...
                cph_test_patches,
                cph_test_labels,
            ) = generate_patch_data(cph_nmf_data, msi_cph, 1, patch_size=patch_size)
            (
                naive_train_patches,
                naive_train_labels,
                naive_test_patches,
                naive_test_labels,
            ) = generate_patch_data(naive_nmf_data, msi_naive, 0, patch_size=patch_size)

            # perform svm
            # fn = f"patch_{patch_dim[0]}_pca_{n_comp}_train_75.npy"
            ...
            # assert train_patches.shape == (patch_dim * patch_dim, n_comp)
            # continue
            ...
            # labels = data['labels']
            param_grid = {
                "C": np.arange(8.41, 8.43, 0.001),
                "gamma": np.arange(0.79, 0.81, 0.001),
                "kernel": ["rbf"],
            }  # , 'poly', 'sigmoid']}
            ...
            print("accuracy: ", acc)
            acc_list.append(acc)
    np.save("pca_19_acc_patch_size_5_31_5.npy", acc_list)
    pass
