...
def fit_best_svm(patches, labels, test_size=None, param_grid=None):
    ...
    print("patch size: ", np.shape(patches))
...
if __name__ == "__main__":
    ...
    for n_comp in n_comp_list:
        ...
        np.save("nmf_neg_20_transformed_data.npy", [cph_nmf_data, naive_nmf_data])
        ## make roi of dimension (15, 15)
        patch_list = np.arange(5, 35, 5)
        acc_list = []
        for patch in patch_list:
            print("patch shape: ({}, {})".format(patch, patch))
            (
                cph_train_patches,
                cph_train_labels,
                cph_test_patches,
                cph_test_labels,
            ) = generate_patch_data(cph_nmf_data, msi_cph, 1, patch_size=(patch, patch))
            ...
            (
                ...
                naive_nmf_data, msi_naive, 0, patch_size=(patch, patch)
            )
            ...
            # labels = data['labels']
            param_grid = {
                "C": np.arange(8.41, 8.42, 0.01),
                "gamma": np.arange(0.79, 0.798, 0.01),
                "kernel": ["rbf"],
            }  # , 'poly', 'sigmoid']}
            ...
            # np.save("acc_vs_patches.npy", acc_list)
            np.save("nmf_acc_patch_size_5_31_5.npy", acc_list)
    pass
